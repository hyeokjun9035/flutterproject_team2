...
function midTmFc(dt) {
  ...
  return `${kst.getUTCFullYear()}${pad2(kst.getUTCMonth() + 1)}${pad2(kst.getUTCDate())}${pad2(baseH)}00`;
}
...
function regIdTaFromAdmin(adminArea) {
  const s = String(adminArea ?? "").replace(/\s/g, "");
  if (s.includes("서울")) return "11B10101";
  if (s.includes("인천")) return "11B20201";
  return null;
}
...
exports.getDashboard = onCall({ region: "asia-northeast3" }, async (request) => {
  try {
    ...
    const [midLand, midTa] = await Promise.all([landRegId ? safe(callMidLand(landRegId, tmFc), null) : null, taRegId ? safe(callMidTa(taRegId, tmFc), null) : null]);
